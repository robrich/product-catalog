...
export async function getUsersByPage(req: Request, res: Response) {
  ...
  const [userRows/*, fields*/] = await db.query<RowDataPacket[]>('select `user` from information_schema.users where is_deleted = 0 order by `user` limit ? offset ?', [PAGE_SIZE, (PAGE_SIZE * pageNum)]);
